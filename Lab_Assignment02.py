...
def spawn_diamond():
    ...
    diamond_x = random.randint(L_EDGE + DIAMOND_RADIUS, R_EDGE - DIAMOND_RADIUS)
    diamond_y = T_EDGE - 55
   ...
   # """Advance the game using delta time, then request the next rendered frame."""
    ...
    if not game_over and not paused:
        if cheat_code:
            ...
            if abs(target_x - catcher_x) <= catch_step:
                catcher_x = target_x
            else:
                catcher_x += catch_step if target_x > catcher_x else -catch_step
...
def mouse_call(button, state, x, y):
    ...
    click_x = x - WIDTH / 2
    click_y = HEIGHT / 2 - y
    if -285 <= click_x <= -210 and 275 <= click_y <= 335:
        reset_game()
    elif -45 <= click_x <= 45 and 275 <= click_y <= 335 and not game_over:
        paused = not paused
        last_time = time.perf_counter()
    elif 225 <= click_x <= 295 and 275 <= click_y <= 335:
...
def setup_projection():
    ...
    glLoadIdentity()
    glOrtho(L_EDGE, R_EDGE, B_EDGE, T_EDGE, 0, 1)
